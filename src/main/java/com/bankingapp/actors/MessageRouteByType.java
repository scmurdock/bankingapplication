...
public class MessageRouteByType extends UntypedActor {
    ...
    private ActorRef kafkaTopicProducerActor = getContext().actorOf(new RoundRobinPool(1).props(Props.create(KafkaTopicProducerActor.class)));//a pool of one means that only one actor is running at any moment and if it crashes, the actor restarts
